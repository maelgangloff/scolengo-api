...
export class AbsenceFilesResponse extends Array<AbsenceFile> {
  ...
  public toCSV(): string {
    return (
      'created,type,status,start,end,reason,reason_label\n' +
      this.map((file) => {
        const {
          creationDateTime,
          absenceType,
        ...
        return `${creationDateTime},${absenceType},${absenceFileStatus},${absenceStartDateTime},${absenceEndDateTime},"${
          absenceReason?.code ?? ''
        }","${absenceReason?.longLabel ?? ''}"`
      }).join('\n')
    )
  }
}
