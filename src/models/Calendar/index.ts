...
export class AgendaResponse extends Array<Agenda> {
  ...
  public static lessonToVEVENT(
    lesson: Lesson,
    dtstamp: Date = new Date()
  ): string {
    const convertDT = (d: string): string =>
      new Date(d).toISOString().replace(/[-:.]/g, '').slice(0, -4) + 'Z'
    const {
      id,
      canceled,
      title,
...
DESCRIPTION:${subject.label} avec ${teachers
      .map((t) => `${t.title ?? ''} ${t.firstName} ${t.lastName}`)
      .join('; ')}
LOCATION:${location}
END:VEVENT`
  }
...
X-WR-CALNAME:${calendarName}\n` +
      lessons
        .map((lesson) => AgendaResponse.lessonToVEVENT(lesson, dtstamp))
        .join('\n') +
      '\nEND:VCALENDAR\n'
    )
  }
